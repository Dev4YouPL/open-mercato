...
export async function POST(
  ...
  try {
    const result = await prepared.container
      ...
      .execute("supplier_demo.supply_case.approve_counter", {
        input: {
          caseId: id,
...
export const openApi = {
  ...
  methods: {
    POST: {
      summary: "Approve a supplier counter recommendation",
      tags: ["Supplier Demo"],
      requestBody: { schema: bodySchema },
      responses: [
        { status: 202, description: "Revision queued" },
        { status: 409, description: "Case changed" },
        { status: 422, description: "Recommendation required" },
      ],
    },
  },
};
