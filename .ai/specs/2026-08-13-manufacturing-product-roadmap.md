...
Catalog remains the UoM master. Manufacturing consumes exact quantity, conversion and immutable snapshot semantics only through a stable Catalog public contract. This is a capability-level contract, not a Manufacturing programme dependency: Catalog delivery is not owned, tracked, or waited on by Manufacturing. It does not gate package bootstrap, Work Center and routing drafts, the neutral fact ledger, or other non-quantity work. Only quantity-bearing BOM, definition-release, and production-order paths consume the accepted contract. WMS owns its storage precision, movement evidence, reversal and generic posting contracts; stock-affecting execution consumes those accepted public contracts rather than redefining them. The first discrete core owns its bounded `yieldFactor` rule; broader process loss, potency, tolerance, and variance policies remain with their specialist manufacturing capabilities.
