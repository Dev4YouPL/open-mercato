...
> **Proposed first-release focus (2026-09-05):** [`Manufacturing End-to-End MVP`](2026-09-05-manufacturing-end-to-end-mvp.md) defines the candidate first OSS release as an extensible CRUD-first product slice with one manually controlled flow: bounded multi-level BOM -> released occurrence snapshot -> single-step production order with direct-occurrence execution -> guarded material issue -> full output receipt -> compensating correction. This roadmap remains normative for ownership and post-MVP architecture. Catalog UoM redesign, WMS Site/precision/evidence changes, generic atomic posting groups, generalized policy validation, and the remaining Wave 0 scope are follow-on options, not MVP prerequisites. The MVP adds only a narrow typed WMS posting port; it does not refactor WMS data or behavior.

> **Why:** a BOM-only release does not validate production value, while completing all Wave 0 foundations before release delays real-user learning and changes multiple modules before demand is proven. The MVP intentionally validates one complete business transaction with Manufacturing-owned workflow plus the narrow WMS posting port. After that release, usage evidence determines which capability from this roadmap should be selected next; the order below is not a delivery commitment.
...
Manufacturing readiness is staged. The narrow MVP follows its three dedicated child contracts and requires neither Site, routing/Work Centers, Catalog normalization redesign, WMS precision/evidence migrations, nor a generic atomic posting group. The broader Wave 0 target retains those dependency gates when its wider capabilities are selected after MVP evidence is reviewed.
...
Before MVP stock execution is approved, its child contracts must prove that postings fit the current supported quantity envelope, are idempotent, guarded, scoped, durably compensatable, and reconcilable; stock cannot be consumed twice; and accepted actions retain auditable history. Temporary partial issue is allowed only as a visible, retryable or compensatable state. The broader Wave 0 target may additionally require exact cross-unit arithmetic, atomic posting groups, and generic reversal contracts when selected.
...
**Status:** Accepted product-roadmap baseline for staged Wave 0 delivery (2026-08-20). Dedicated capability specifications, readiness evidence, and the named safety gates remain required before each implementation slice begins.
...
Within this roadmap, the accepted architecture is the single source of truth for product boundaries and records the governing architectural decisions directly. Historical review notes that were not committed to the repository are not normative dependencies.
...
| [`2026-09-05-manufacturing-end-to-end-mvp.md`](2026-09-05-manufacturing-end-to-end-mvp.md) | Proposed first-release scope and composed acceptance gate | Maintainer review pending; implementation split into three child specs |
...
The platform is discrete-first, but not discrete-only. Wave 0 lives in one opt-in `manufacturing` runtime module. Within it, model-neutral contracts remain separated from the first discrete aggregates so a future process/batch, repetitive, configure-to-order, engineer-to-order, project, subcontract, remanufacturing, or hybrid capability does not inherit semantics that do not fit it. A separate module or public package subpath is introduced only when an independently deployable capability or real second consumer proves the need.
...
Establish the standalone `@open-mercato/manufacturing` package with one opt-in `manufacturing` module and stage readiness per slice. Once the roadmap and a dedicated specification are accepted, draft discrete definitions may be implemented before execution contracts. Released orders and stock effects unlock only after their named minimum safety gates. Preserve model-neutral internal seams, place authoritative concepts in existing shared domains, capture immutable Manufacturing facts, and classify every inter-module relationship explicitly.
...
- The narrow MVP may snapshot selected existing warehouses and locations directly. The broader Wave 0 target introduces WMS-owned site/plant identity and current site-to-warehouse role assignments before site-aware released definitions or orders.
...
- Production issues, returns, backflushes, scrap, and receipts are Manufacturing use cases. The narrow MVP submits guarded semantic postings through its typed WMS port; normal production movements must not be classified as `manual`. The broader Wave 0 target replaces per-intent orchestration with a generic atomic WMS posting-group contract when selected.
...
- Every manufacturing definition used for execution is released atomically and versioned. The narrow MVP freezes a bounded multi-level occurrence tree, deterministic child-revision IDs, and selected warehouse/location IDs, while each order executes only its top-level direct occurrences. Later site-aware profiles additionally select the applicable definition by item/variant, site, and `plannedStartDate`.
...
- The first Manufacturing UI is limited to list/detail, create/edit, release, confirm, and reverse flows. Bulk actions, advanced analytics, approval workflows, and segregation-of-duties policy are later capabilities; MVP ACL is bounded to view, manage, execute, and reverse. Post-Wave 0 P1.4c separately evaluates a Sales-level BOM list workspace with search, filters and personal perspectives; it is not a Gate A requirement.
...
- Tenant-defined custom fields are supported on WMS Site in the first foundation. Manufacturing BOM/order custom fields are a later extension, not a draft/release gate. Post-Wave 0 P1.4h owns the decision for BOM custom fields, tags and controlled document references.
- Basic instructions and attachment references may be frozen into a released snapshot. Controlled document packages, approval/signature semantics, and full document-control lifecycle remain separate capabilities; P1.4h must reuse an approved provider rather than create a new file-storage system.
- Public contracts follow `BACKWARD_COMPATIBILITY.md`; additive changes must also be operationally backward compatible.
- The standalone `@open-mercato/manufacturing` package initially exposes one runtime module, `manufacturing`, hard-requiring only `catalog`. WMS, `resources`, and `planner` are optional peers. Model-neutral contracts remain internal until a real second consumer justifies an additive public subpath or module boundary; future specialist capabilities must not depend on discrete-specific internals.
...
The following decisions resolve thirteen contract-shaping questions and three governance notes. They are binding on the relevant readiness slice and later capability specifications.

| # | Decision | Candidate rule |
|---|---|---|
| C1 | One initial runtime module with guarded internal boundaries | The standalone `@open-mercato/manufacturing` package ships one opt-in runtime module, `manufacturing`. Wave 0 capabilities P1.4a/P1.4b and P1.5–P1.11, plus later P1.4c–P1.4h BOM capabilities, live inside it. Model-neutral seams remain internally separate from discrete aggregates; a future public subpath or specialist module requires an independently validated consumer and must not depend on discrete internals. |
...
| C3 | Basic identity first; advanced number ranges later | UUID is canonical identity. `manufacturing` provides a simple concurrency-safe site-scoped order display number. Lot/serial values may be supplied explicitly and WMS validates uniqueness. P1.13 later adds configurable order/batch/lot/serial formats, resets, generated values, block reservation, and offline allocation without blocking the MVP. |
| H1 | Backflush is a later Manufacturing mode over generic WMS postings | The narrow MVP supports explicit full issue only. When selected after MVP evidence, backflush derives cumulative issue lines from the immutable released-definition snapshot and submits them through the generic atomic WMS posting-group contract. WMS never reads Manufacturing definitions or calculates backflush quantities. |
| H2 | Multi-level definitions with direct-occurrence execution belong to the MVP | The narrow MVP freezes and displays a bounded multi-level definition but executes only the top-level revision's direct occurrences. A direct `stock` line and a direct `produce` line are both issued as stocked component variants; `produce` does not recurse or create a child order automatically. Producing that subassembly uses a separate manually created, optionally parent-linked order with receive-to-stock followed by explicit parent issue. Automatic child-order networks and recursive execution remain later capabilities. |
...
### Internal model-neutral boundary (C1)

The single `manufacturing` module keeps the following model-neutral concepts in an internal boundary, separate from discrete aggregates:
...
- the abstract released-definition and order/operation lifecycle contracts (state machine), implemented by capability-specific aggregates;
- the minimum append-only fact contract, correlation/causation IDs, and reversal/compensation semantics;
- the provider-neutral interface through which model-specific capabilities submit already-derived physical stock effects to a generic WMS posting group, plus the MES/edge confirmation interface;
- a model-neutral component-structure occurrence and explosion contract, including cycle detection and stable occurrence paths in released/execution snapshots; the first discrete capability specializes it as a BOM and a future process capability may specialize it as a formula/recipe structure;
...
This internal boundary owns no discrete-only or process-only aggregate. Discrete `ProductionDefinition`/`ProductionOrder` and future process `Formula`/`Recipe`/`BatchOrder`/`Campaign` remain capability-specific implementations. Internal imports must point from capability implementations toward model-neutral contracts, never the reverse.

The boundary is an implementation acceptance criterion, not a second runtime module. It contains shared contracts, lifecycle primitives, fact schemas, and provider interfaces only. It owns no UI, workflow orchestration, direct WMS calls, model-specific business logic or aggregate, and it does not implement a reserved seam until a real consumer requires that behavior. Model-specific adapters may call optional providers, but model-neutral code cannot become an integration host or a second application layer. P1.0a exports no domain contracts; any future reusable package subpath is additive and separately specified.

### Module ID and capability naming policy

The initial developer-facing runtime module ID is `manufacturing`. A later independently deployable capability may use `manufacturing_*` only when it would be meaningless without Manufacturing and after its own specification proves the module boundary. User-facing labels may still say “Production orders”, “Production planning”, or another familiar business term through i18n.
...
In the initial Site capability a warehouse may serve several roles but may belong to only one active site. Shared warehouses across active sites require a later production-network/shared-stock policy. Site is outside the narrow MVP. Scheduled/effective-dated assignment changes and site timezone/calendars remain later capabilities.

### 2. Resource, work-center, asset, staff, and calendar ownership

Reusable resource identity, capacity unit, and active state remain in `resources`; reusable timezone-aware availability rules remain in `planner`. The narrow MVP has no routing, Work Center, capacity, or calendar dependency. Manufacturing owns minimal work centres and routing applicability linked by scalar IDs only if that follow-on capability is selected.
...
The internal model-neutral boundary in `manufacturing` defines release identity, site, product/item and UoM references, effectivity, quantity/configuration applicability, readiness, document package identity, and immutable revision semantics. It encodes no model-specific aggregate and is not a separate runtime module or public export in P1.0a.
...
The narrow MVP permits a separately created subassembly order to carry an optional explicit parent-order reference. Each order still executes only its own revision's direct occurrences: child output is received to WMS stock and then explicitly issued to the parent. Recursive execution inside one order, direct issue, automatic child-order creation, MRP-driven order networks, and phantom behavior remain later capabilities.

Every BOM draft revision declares its output item/variant and base output quantity/UoM; P1.7 adds release status, Site applicability and effectivity to the released-definition lifecycle. Every BOM line declares exact component quantity/UoM, `consumptionBasis` (`variable` or `fixed`), and `yieldFactor` greater than `0` and at most `1` (default `1`). A reusable BOM stores no customer, sales-order or required/planned date. P1.10 owns provider-neutral demand source (`sourceType/sourceId/sourceLineId`), `requiredByAt`, planned dates and the selected execution snapshot; any future ETO/order-specific BOM is a separate snapshot/overlay. Gross requirement equals nominal requirement divided by yield. `fixed` is planned once per production order and occurrence and may be issued in parts; first qualifying backflush posts its remaining amount. `variable` backflush calculates a cumulative target from accepted `good + scrap` at the occurrence's operation/order backflush point and posts only the delta from already posted net consumption. Exact cumulative arithmetic avoids per-confirmation drift. Reversal copies and negates persisted posting values rather than recalculating them.
...
Lot/serial-controlled inputs and outputs are outside the narrow MVP. A later capability may accept an explicit lot/serial value validated by WMS; automatic generation, configurable formats/resets, pre-assigned offline pools, and block allocation remain separate options.
...
Catalog remains the UoM master. The narrow MVP accepts only values already expressed in the current WMS inventory unit and proven representable by the current arithmetic/storage envelope; it performs no cross-unit conversion and does not wait for the Catalog normalization redesign. Later cross-unit and broader quantity-bearing capabilities consume the stable Catalog exact-quantity contract. WMS owns storage, movement evidence, and posting behavior; the MVP consumes its additive typed posting port, while broader execution may later require precision, generic posting-group, and reversal contracts.
...
The narrow MVP exposes explicit full material issue, full output receipt, and evidence-derived correction; it has no partial, scrap, operation, or backflush confirmation model. A later confirmation capability may add those dimensions. UI and API always use the same canonical command, and external data becomes ERP truth only after Manufacturing validates and accepts it.
...
| Work center and routing constraints | `manufacturing` | Own semantics and applicability while keeping model-specific rules internally separated | Extensions over optional shared resource IDs |
...
| Minimum manufacturing facts | `manufacturing` model-neutral boundary | Append, correlate, reverse/compensate | Advanced modules consume facts through separately approved contracts |
...
| Manufacturing package/module | `manufacturing` in standalone `@open-mercato/manufacturing` | Own Wave 0 capabilities with guarded internal model-neutral seams | Hard-requires only Catalog; optional peers and future models must not import discrete internals |
...
| Production display numbers | `manufacturing` | Generate a simple concurrency-safe site-scoped number over canonical UUID identity | Configurable formats/resets and offline ranges move to P1.13 |
| Output lot/serial identity | WMS validates authoritative inventory identity | Excluded from narrow MVP; a later profile may supply and snapshot an explicit value | Automatic generation and offline pools remain a separate option |
...
| Temporal/effectivity authority | `manufacturing` model-neutral boundary | Stamp date effectivity on definitions and `recordedAt`/`occurredAt` on facts | Full bitemporal/as-of analytics are later |
...
### Shared foundation and `manufacturing`
...
| WMS sites | Stable, custom-field-extensible site identity and closed current warehouse-role assignments | Existing WMS requirements; site contract itself adds no Manufacturing requirement | Manufacturing consumers, directory, network planning | Outside narrow MVP; initial Site profile is inactive by default and restricts a warehouse to one active site | External OSS `wms` capability; setup-once UI remains deliberately minimal |
...
| Resources and calendars | Resource identity/capacity and planner availability rules | Current code: `resources` requires `planner` | Assets, workforce, manufacturing extensions | Outside narrow MVP; later manual routed profiles may omit calendar enforcement | Existing foundations; minimal work-centre boundary freezes before released routing, advanced calendars before scheduling |
...
| Capability | Owned data | Hard runtime requirements | Soft integrations/providers | Snapshot/fallback when absent | Placement/licensing |
|---|---|---|---|---|---|
| `manufacturing` discrete Wave 0 capabilities | Discrete `ProductionDefinition`, BOM/routing releases, `ProductionOrder`, operation intent and manufacturing transactions | Internal model-neutral contracts; compatible generic WMS posting contract only for stock-affecting execution | Resources/planner, documents, workflows, quality, execution, costing | Drafts and non-stock lifecycle remain separable; manual confirmations work without MES/QMS/APS | Lives in the single OSS `manufacturing` module; P1.4a/P1.4b and P1.5–P1.11 remain separate specs |
...
| Capability | Owned data | Hard runtime requirements | Soft integrations/providers | Snapshot/fallback when absent | Placement/licensing |
|---|---|---|---|---|---|
| Process/batch manufacturing capability | Formula/recipe releases, batch orders, campaigns, yields, co-/by-products, potency and parameters | Separately approved model-neutral contracts and compatible generic WMS postings | Quality, LIMS, traceability, execution, planning | First valid batch need not hard-require advanced QMS/LIMS/traceability modules | Future specification decides whether this remains internal or becomes an optional module; it must not depend on discrete aggregates |
...
| Repetitive manufacturing capability | Rate-based flow, takt, kanban and line-side execution | Separately approved model-neutral contracts and WMS postings | Planning and execution | Discrete order flow remains | Future placement decision; no dependency on discrete aggregates |
| Remanufacturing capability | Disassembly, inspection, repair, recovery and returned-asset genealogy | Separately approved model-neutral contracts | Quality, traceability and assets | Standard production remains | Future placement decision; no dependency on discrete aggregates |
| Additive manufacturing capability | Build preparation, powder/material tracking and post-processing | Separately approved model-neutral contracts | Execution, connectivity and traceability | No additive behavior when absent | Research candidate; placement not reserved |
...
organization/auth + catalog + WMS sites + simple manufacturing order number
       |                 |                  |
       |                 |                  +--> resources (post-MVP routing/scheduling option)
...
@open-mercato/manufacturing
       |
       v
manufacturing (one opt-in runtime module)
       |-- internal model-neutral contracts, facts, and provider seams
       |-- Wave 0 discrete capabilities (P1.4a/P1.4b and P1.5–P1.11)
       |-- future specialist capability may consume an approved neutral seam,
       |   but never discrete-specific internals
                  |
...
|---|---|
| Discrete assembly, MTS, MTO | `manufacturing`, engineering, WMS, optional planning/scheduling/execution |
| Repetitive and lean | Approved model-neutral Manufacturing contracts, repetitive capability, WMS, planning and execution |
| Process and batch | Approved model-neutral Manufacturing contracts, process capability, WMS, optional quality/LIMS/traceability/execution/planning |
...
| Hybrid/mixed-mode | Composition of model-specific capabilities over approved neutral contracts; never a forced inheritance from discrete aggregates |
...
- No future specialist manufacturing capability or module declares a runtime requirement on discrete-specific aggregates or internals in `manufacturing`.
...
| `manufacturing` | Discrete-first definitions and order lifecycle | Maintain released BOMs/routings; define work centers and operations; create production orders; derive material issue, return, backflush, scrap, and output lines for generic WMS posting groups. |
...
| Module / capability | Business responsibility | What users can do |
|---|---|---|
| Process/batch capability | Process, batch, and continuous manufacturing | Maintain released formulas/recipes; run batches/campaigns; manage yield, co-/by-products, potency, parameters, and batch genealogy. |
| Configure-to-order capability | Configure-to-order manufacturing | Define options and constraint rules; validate a configuration; derive the applicable released BOM/routing. |
| Engineer-to-order/project capability | Engineer-to-order/project manufacturing | Link work to projects, milestones, project-specific supply, operational WIP, resources, and cost. |
| Repetitive capability | Lean/repetitive manufacturing | Operate takt, kanban, rate-based flows, line-side replenishment, and repetitive execution. |
| Remanufacturing capability | Repair, recovery, and remanufacturing | Disassemble, inspect, repair, replace, recover, and reuse components while retaining configuration/history. |
| Additive capability | Additive manufacturing | Prepare build jobs; track powder/material, post-processing, and machine-level execution. |
...
- Do not put MRP, APS, MES, QMS, costing, process recipes, or client-specific substitution policy into the Wave 0 `manufacturing` scope.
...
| Discrete and future-model boundaries | Pass | Wave 0 uses one runtime module but keeps model-neutral contracts isolated from discrete aggregates; future extraction is evidence-driven. |
...
**Roadmap architecture is accepted as the ownership and broader Wave 0 baseline.** The proposed narrow MVP uses its own MVP-D/MVP-O/MVP-X readiness lane and, once accepted, supersedes Gate B/C prerequisites only for that restricted profile. Broader released-order and stock-execution work retains the original Gate B/C dependencies. No post-MVP capability has an automatic delivery priority.

## Changelog

- 2026-09-05: Aligned H2 and the release law with the MVP's bounded multi-level snapshot and direct `stock`/stocked-`produce` execution boundary.
- 2026-09-05: Added the end-to-end MVP as the proposed first-release boundary, split it into three child contracts, and retained Wave 0 as an evidence-prioritized future option map.
...
- 2026-08-13 (Revision 3): Added the "Wave 0 Contract Decisions" section resolving C1–C3, H1–H6, M1–M4, and S1–S3; the initial separate-kernel direction was later superseded by one `manufacturing` runtime module in `@open-mercato/manufacturing`.
...
- 2026-08-19: Initially explored a `manufacturing_*` family, then accepted one durable runtime ID, `manufacturing`. Applied the prefix test to later optional capabilities; the illustrative catalogue does not reserve module IDs without an independently validated boundary.
- 2026-08-19: Made the internal model-neutral guardrail executable: contracts, lifecycle primitives, facts, and provider interfaces only; no UI, orchestration, direct WMS calls, model-specific aggregates, speculative seam implementations, or P1.0a domain exports.
- 2026-08-19: Consolidated the proposed base/discrete runtime split into one opt-in `manufacturing` module. The module hard-requires `catalog`; WMS, Resources, and Planner remain optional. P1.4a/P1.4b and P1.5–P1.11 remain separate capability specifications inside the module, and future specialist models may consume only separately approved neutral contracts.
- 2026-08-19: Split P1.4 after fresh-context review and roadmap-owner approval: P1.4a owns direct-level BOM authoring/integrity, while P1.4b owns bounded read-only recursive preview. Only P1.4a is required by P1.7 release work.
