...
- Every manufacturing definition used for execution is released atomically and versioned. The narrow MVP freezes one direct definition and selected warehouse/location IDs. Later multi-level and site-aware profiles additionally select child BOM revisions and the applicable definition by item/variant, site, and `plannedStartDate`.
...
| H2 | Multi-level BOMs and manual order links belong to the broader discrete core | The narrow MVP executes direct `stock` lines only. A later multi-level capability may let a subassembly be stock-supplied or produced by a manually linked child order, with receive-to-stock followed by explicit parent issue. |
