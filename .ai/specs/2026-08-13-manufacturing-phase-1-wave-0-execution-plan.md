...
**Relationship to roadmap readiness:** The parent roadmap defines Gate A (drafts), Gate B (released orders), Gate C (stock execution), and deferred capability gates. This document groups Manufacturing-owned deliverables without taking ownership of Catalog or WMS delivery work.
...
| P1.4g | Copy a BOM | `manufacturing` post-Wave 0 authoring capability | P1.4a commands/graph; Catalog quantity/UoM contract | Specification task [#5412](https://github.com/open-mercato/open-mercato/issues/5412); not a Wave 0 gate | P1.4a, Catalog contract | One explicit copy command with new IDs and full scope/Catalog/quantity/graph revalidation; no import/export |
...
| P1.5a | Establish routing families and initial drafts | `manufacturing` discrete definition capability | shared `CrudForm.mutationResource` support | Yes after P1.0a and dedicated spec readiness | P1.0a plus shared mutation-resource support | Product-neutral family/revision identity, aggregate CRUD, revision-root locking and undo; no operations or Work Centre reference |
...
| P1.5b | Author append-ordered routing operations | `manufacturing` discrete definition capability | P1.5a and P1.6 | Yes after both prerequisite implementations | P1.5a and P1.6 | Optional single-sequence operations with Work Centre, setup/run time, and instructions; no scheduling semantics |
| P1.5c | Reorder routing operations | `manufacturing` discrete definition capability | P1.5b | Yes after P1.5b | P1.5b | Atomic bounded manual reorder and renumbering; no scheduling semantics |
| P1.7 | Define released-definition lifecycle and immutable definition snapshots | `manufacturing` | External WMS `Site` contract, Catalog public quantity/UoM contract, attachments, accepted BOM/routing contracts | Specification preparation after upstream shapes are known; implementation follows Gate B prerequisites | External WMS Site contract, Catalog contract, P1.4a, P1.5a, P1.5b, P1.6; P1.5c is optional | Child revisions and occurrence-preserving multi-level definition snapshots frozen atomically at definition release; no order release; P1.4b is not a release prerequisite |
| P1.8b | Define the Manufacturing inventory posting adapter | `manufacturing` | External WMS posting provider, P1.9 fact writer, P1.10 execution/confirmation contracts | Semantic-command preparation only | External WMS posting contract, P1.9, P1.10 | Manufacturing-derived issue, return, cumulative backflush, output, scrap and reversal orchestration without WMS learning Manufacturing vocabulary |
...
| P1.10 | Add the first discrete production-order lifecycle, execution snapshot and basic confirmations | `manufacturing` | External WMS `Site` contract, released definitions | No as a shippable feature until Gate B passes | External WMS Site contract, Catalog contract, P1.7, P1.9 | `draft → released → in_progress → completed/cancelled`, provider-neutral demand source (`sourceType/sourceId/sourceLineId`), `requiredByAt` and planned dates, top-level definition selection, immutable execution snapshot, simple order number, `complete_short`, stock-free partial/final/scrap confirmations and corrections |
| P1.11 | Add stock-affecting production execution | `manufacturing` | optional `wms`, WMS `Site` | No | External WMS quantity, evidence and posting contracts; P1.8b, P1.9–P1.10 | Manufacturing-derived issue/return/backflush/output/scrap groups, exact reversal, explicit lot/serial, and on-demand reconciliation |
...
| Routing family and initial draft CRUD | P1.5a | It establishes product-neutral family/revision identity and revision-root concurrency without Work Centre or operation dependencies. |
| Optional sequential routing operation CRUD | P1.5b, with reorder in P1.5c | It stores operation order, basic setup/run time, instructions, Work Centre references, and no calendar/scheduling semantics. |
...
|---|---|
| Release of a manufacturing definition | active `siteId`, raw-material and finished-goods defaults, current warehouse-role snapshot, deterministic child revision selection, base-output/fixed/variable/yield semantics, Catalog quantity/UoM contract, work-centre ownership, and immutable definition snapshot |
...
| Component issue, return, and backflush | External generic WMS posting contract, Manufacturing semantic adapter, and idempotency/reversal semantics |
| Output receipt, explicit lot/serial identity, and scrap | WMS atomic posting contract, WMS uniqueness validation, and WMS precision/evidence rules |
...
|---|---|---|
| First parallel increment | P1.0a bootstrap, P1.4a BOM authoring, P1.4b bounded preview, P1.5a routing family/draft foundation, P1.6 work-centre boundary | Delivers the Manufacturing foundation and product-neutral routing identity early; the Catalog public contract supplies quantity semantics, while P1.5a and P1.6 can proceed independently after P1.0a. |
| Routing-operation increment | P1.5b append operation authoring, then optional P1.5c manual reorder | P1.6 gates Work Centre-backed operation authoring, not the P1.5a family/draft foundation. |
...
| Stock-readiness increment | External WMS quantity, evidence and posting contracts, then P1.8b Manufacturing adapter | These are the only additional blockers for physical execution |
...
| WMS learns Manufacturing definitions or production-specific movement enums | High | P1.8b derives concrete lines; the external WMS provider accepts generic atomic groups with opaque namespaced references and durable display fallback | The external contract must define registration/disabled-module display/provider compatibility, and P1.8b must preserve semantic ownership |
...
1. WMS proves non-narrowing storage, immutable quantity/UoM evidence, and exact reversal before production postings are enabled.
2. The WMS provider accepts a generic atomic idempotent posting group of concrete issue/receipt/transfer/write-off/reversal lines with opaque namespaced source/reason references; it returns posting ID, quantity, UoM, and `postedAt`. `valuationContextRef` remains optional until costing defines it.
...
Disabled optional modules and every changed API/UI path still require proportionate integration coverage.
...
- 2026-08-13: Recorded the WMS Site design with current-only assignments, one default warehouse per configured role, and immutable consumer snapshots; scheduled/effective-dated assignments and site timezone are later features. Identified advanced production number ranges as a separate follow-up capability.
- 2026-08-13: Refined the external WMS Site UI baseline for infrequent setup: full canonical custom fields and field injection on `Site`, no custom fields on assignments, minimalist DataTables with stable extension hosts, and no built-in CRM-scale search/filter/view/export/bulk controls.
...
- 2026-08-28: Removed the unrelated WMS/Sales packaging concern from the Manufacturing execution plan; any future work on it needs its own owner, scope and specification.
...
- 2026-08-30: Refined P1.5 into independently deployable P1.5a family/initial-draft, P1.5b append-operation, and P1.5c reorder slices. P1.5a may proceed after P1.0a and shared mutation-resource support; P1.6 gates P1.5b rather than P1.5a.
