...
- BOM list free-text search and server-side sorting (P1.4c — the shipped list narrows only by the API-supported product/variant filters, through the shared Filters overlay, column chooser and personal perspectives), business identity (P1.4d), history/comments (P1.4e), copy (P1.4g), tags/attachments/document control and revision-/line-level metadata (P1.4h — only the family-level custom-field attachment point ships here), import/export, bulk actions, and global cross-module search.
...
All routes require auth, concrete organization, zod, feature metadata, scoped queries, exported `openApi`, and custom-route mutation guards. Before any data access they call Directory's `organizationScopeService.resolveConcreteForRequest` DI boundary; `selectionRejected` or the absence of a concrete `selectedId` returns the platform `organization_selection_invalid` response. The service may select implicitly only when the effective scope, or an unrestricted tenant lookup bounded to two rows, proves there is exactly one active organization. It never chooses a home organization from an ambiguous multi-organization scope. Path IDs and authenticated tenant/organization authority overwrite any guard-transformed body values before command dispatch. Successful changed mutations return `x-om-operation` and the new aggregate `updatedAt`.
...
| P1.4a B4 — UI and i18n | done | B2/B3 | AC-UI | `yarn workspace @open-mercato/manufacturing build`; `yarn i18n:check-sync`; browser walkthrough (2026-09-01) | BOM list/create/editor paths render and the authoring flow was exercised in a real browser |
...
- [x] Real-browser walkthrough (2026-09-01, dev server on `:3000`, superadmin, `Acme Corp`): list → detail → add line → duplicate occurrence → cycle rejection → reorder → delete confirmation → stale-token conflict banner → create-page validation, plus a mobile-viewport smoke. Four defects were found and fixed in the same pass — see the changelog entry for 2026-09-01.
- [x] Catalog enrichment is implemented against `QueryEngine` (`lib/bom/catalog-enrichment.ts`); list, detail and line rows show resolved product/variant labels and fall back to `catalogState:'missing'` only when Catalog cannot be read. The earlier "returns missing unconditionally" note is obsolete.
- [ ] Client-file LOC budgets are still exceeded by two leaves: `BomLinesEditor.tsx` is 431 LOC and `BomListClient.tsx` is 334 LOC against the spec's `<=300` budget. `BomHeaderFormClient` (224), `BomLineDialog` (212) and `BomEditorClient` (149) are within budget.
- [ ] NOT DONE: automated coverage for the seven commands, the ten API routes, undo/redo, cycle prevention, cursors, positions and the UI leaves; `.ai/qa/tests` ships no Manufacturing integration test. B5's `O(V+E)` graph benchmark at 1,000 families/10,000 edges and the extended module-isolation/disabled-peer fixtures were not added.
- [ ] NOT DONE: named check/FK constraint and partial-unique index names are not mapped to domain codes, so a constraint violation that escapes the application-level checks surfaces as a generic 500 rather than its stable `bom.*` code.
- [ ] Deviation: a malformed or replayed keyset cursor returns an empty page instead of the specified rejection. The behaviour is scope-safe (no cross-tenant read, no skip/duplicate traversal) but a client bug is indistinguishable from "no more results".

## Changelog

- 2026-09-01: First real-browser QA pass over the shipped P1.4a authoring flow. Verified: list/detail rendering with Catalog-enriched labels, direct-line columns (entered and normalized quantity, basis, yield, supply, resolution), duplicate occurrences kept as separate rows, direct self-reference rejected before save, delete confirmation naming the occurrence ordinal, `bom.target_conflict` on an exact duplicate target, `bom.variant_product_mismatch` on a foreign variant, decimal-string-only quantity input, `limit<=100`, and the mobile viewport. Four defects were found and fixed: (1) `manufacturing.bom_line.reorder` returned 500 for every swap because the `bigint` position hydrated as a native BigInt and broke action-log JSON serialisation, committing the swap while reporting failure and recording no undo entry — the column is now read back as a string; (2) the aggregate optimistic-lock conflict returned a BOM-specific code instead of the platform `record_modified`/`optimistic_lock_conflict` body, so the shared record-changed banner never appeared; (3) the BOM line dialog's `Cmd/Ctrl+Enter` handler was a no-op; (4) unexpected route errors were swallowed into an unlogged generic 500. Yield-factor range `(0, 1]` is now enforced in the command layer as `bom.quantity_invalid` (422) instead of relying on the database check constraint.
...
- 2026-08-31: Aligned the shipped BOM UI with the platform list/form conventions. The family list moved its product/variant narrowing into the shared `FilterBar` Filters overlay (combobox filters plus removable chips) and gained the column chooser, column resizing and personal perspectives; the keyset previous/next controls became a card footer bar shared with the line table. Catalog reference pickers moved from `LookupSelect` media tiles to compact `ComboboxInput` rows (label plus SKU) so a 5000-record Catalog no longer renders as a scrolling gallery. The header form gained grouped two-column layout, and the direct-line table renders basis/supply/resolution as status badges with an explicit normalized-quantity column. Display hygiene: the direct-line `#` column now shows the occurrence ordinal instead of the sparse internal ordering key (1024, 2048, …), and dialogs/confirmations name that same ordinal; canonical decimal strings are trimmed of their scale padding for display and for the text inputs that re-submit them, leaving stored evidence untouched.
- 2026-08-31: Brought the US-BOM-30 family-level custom-field attachment point forward from the P1.4h proposal into P1.4a: `ce.ts` registers `manufacturing:manufacturing_bom` as a custom-field host with no shipped definitions, `createBomSchema`/`updateBomSchema` accept an optional `customFields` map, the create/update commands persist it through the data engine outside the graph-locked transaction and restore it on undo, and the detail response returns the stored values so the `CrudForm` custom-attributes group hydrates. Tags, controlled documents and revision-/line-level metadata remain P1.4h.
- 2026-09-03: Refined the shipped authoring workspace from user review: the family list now uses the standard left-aligned BOM search with Filters and Perspectives aligned right, and its keyset controls render through the DataTable pagination footer even for a single page. The create/edit form is framed as one workspace with a basic BOM data section and optional custom fields in the right column; the user-facing revision-label copy is now Notes while the stable `revisionLabel` persistence/API contract remains unchanged; the component-occurrence dialog uses the larger `max-w-4xl` size.

- 2026-09-06: BOM routes now resolve an authoring-safe organization through Directory's public `organizationScopeService` DI boundary, not a private `core` import. An unrestricted caller without a selected organization may proceed only when a bounded lookup proves its tenant has exactly one active organization; multi-organization and stale/rejected selections remain a 400. Focused service and route-context tests cover the sole-organization, ambiguous, and rejected cases.
